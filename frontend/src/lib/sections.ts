...
export const STRATEGY: Section[] = [
...

  {
    ...
    fields: [
      { path: 'instruments.strike_mode', label: 'strike_mode', type: 'enum', options: ['first_positive', 'automatic', 'custom'], doc: 'How the contract is picked once the first positive tick has decided the side. first_positive buys the strike that ticked (fastest). automatic scores the armed strikes on spread, OI and volume. custom uses strike_reference + strike_offset and ignores which strike ticked.' },
      { path: 'instruments.expiry_rule', label: 'expiry_rule', type: 'enum', options: ['nearest', 'next', 'monthly'], doc: 'Which expiry chain to trade. The physical-settlement roll is applied on top, so a stock still rolls on its last two days whatever this says.' },
      { path: 'instruments.book_score.max_spread_pct', label: 'book_score.max_spread_pct', type: 'float', min: 0, max: 50, doc: 'automatic only — reject a strike whose bid-ask spread is wider than this. A one-sided book counts as infinitely wide.' },
      { path: 'instruments.book_score.min_depth_lots', label: 'book_score.min_depth_lots', type: 'int', min: 0, doc: 'automatic only — minimum traded volume before a strike is considered.' },
      { path: 'instruments.book_score.weight_spread', label: 'book_score.weight_spread', type: 'float', min: 0, doc: 'automatic only — how much a tight spread counts.' },
      { path: 'instruments.book_score.weight_oi', label: 'book_score.weight_oi', type: 'float', min: 0, doc: 'automatic only — how much open interest counts.' },
      { path: 'instruments.book_score.weight_volume', label: 'book_score.weight_volume', type: 'float', min: 0, doc: 'automatic only — how much traded volume counts.' },
      { path: 'instruments.strike_reference', label: 'strike_reference', type: 'enum', options: ['ATM', 'ITM', 'OTM'], doc: 'custom mode only. ITM is a LOWER strike for a CE and a HIGHER strike for a PE; OTM mirrors it.' },
